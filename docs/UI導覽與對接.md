...
- **左 · 即時環境影像**(占畫面 1/3):
  ...
  - **場域 UE 吞吐量**:場域內各 UE(假設 10 台)的吞吐量折線圖,x 為路徑進度;
    各 UE 灰色線當背景,平均線用目前這趟的代表色凸顯,標題列顯示最新平均。
- **右 · 測試狀態總覽**:標題列同一行寫出目前的測試項目(代碼、名稱)與優化是否已開啟。
  - **UAV 測試路徑**(兩台電視寬):同一條航線疊出兩趟軌跡與無人機目前位置;
    下方是目前這趟的任務進度(百分比)與目前階段。
  - **飛行狀態**:相對高度、地速的折線圖(目前這趟,x 軸為航線進度)。
  - **訊號狀態**:SNR、下行速率的折線圖,開啟前 / 開啟後兩條線對照。
- 版面文字避開電視拼接縫。
