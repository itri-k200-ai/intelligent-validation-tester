...
- **左 · 即時狀態**(左卡 144~5616):只有影像與即時數值。影像 3552(2×1728)+ 即時數值 1632;
  1632 是數值格的下限 —— 三欄加欄距要裝得下最寬的「RSRP dBm」(474),再縮值就會被切掉。
  縮窄之後左右分界左移 96,x = 5760 落在右卡的左內距(5712~5808)裡,縫上沒有文字。
...
- **右 · 測試狀態總覽**:標題列寫出測試環境與測試項目。
  - **AMR 測試路徑**(3168 寬):底圖是 51 館 5 樓平面圖(只畫測試用到的範圍,上緣切在走廊北側房間的南牆,依
    `docs/外部文件/前端UI建議/2026-09-15_智慧網路實驗室_室內場域圖51館5樓.pdf` 簡化:外牆、走廊、隔間、
    電梯樓梯與 RU 位置,不畫家具也不寫字);實際串接時改用 SLAM 底圖 + 世界座標 extent,
    上面疊出兩趟軌跡與 AMR 目前位置(有底圖時不畫 RU 標記,所以卡頭也不放 RU 圖例);
    上方是兩段式測試進度(同室外)。
    平面圖座標在 `frontend/src/config/floorPlans.ts`,AMR 路徑點要用同一個座標系。
  - **IM xApp 啟用前後**(2208 寬,與路徑卡約 6:4):吞吐量與平均 SINR 各一張折線圖(兩趟對照),
    名稱自己帶「平均」,小卡標題列不再重複寫一次。
    上格的吞吐量用名稱下面的 **DL / UL** 開關切換,一次只畫一個 —— 兩者量級差很多
    (約 100 kbps 對 4000),疊在一起沒有一條 y 軸讀得準;y 軸與標題列的平均都跟著選到的那個。
    吞吐量放上面那一格(999 高,SINR 708)—— 它是這張卡的重點;SINR 不是這次要改善的目標,
    所以只看趨勢:不標平均差值,兩趟也用同一個字級與亮度(放大「啟用後」會像是被優化過)。
    這個比例的分界不在拼接縫上,所以 x = 9600 會穿過卡內(內容 9135~11217),卡內文字分兩段:
    縫左邊 441 放圖的名稱與 y 軸刻度(441 是「平均 SINR」的下限,欄距因此收成 72,
    間距 9576~9648 仍包住邊框 9594~9606),數值、單位、平均差值與小卡標題都推到縫右邊 9648 起
    (剛好對齊大卡標題列的「測試項目」那一欄);x 軸刻度落在 9285 / 10186 / 11087,也不壓縫。
