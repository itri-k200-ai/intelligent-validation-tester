...
### 室外 UAV 情境 — `/outdoor-scenario`
無人機室外飛行測試(**室外 tester 尚未串接,目前是前端靜態假資料**)。
無人機沿同一條航線飛兩趟:第一趟**優化開啟前**、第二趟**優化開啟後**,一次只跑一個測試項目。
測項清單由左螢幕負責,中牆只專注目前這個測試。**左邊看即時狀態、右邊看測試結果:**
- **左 · 即時狀態**(占畫面 1/3):標題列標示目前優化是否已開啟。
  - 室外固定攝影機、無人機機載攝影機兩個 16:9 影像並排。
  - **飛行狀態**:相對高度 / 地速 / 垂直速度 / 電量 / 衛星數 / 模式(即時數值)。
  - **UAV 訊號**:目前這趟的 SNR / RSSI / RSRQ / 上行 / 下行 / 丟包。
- **右 · 測試結果**:標題列同一行寫出目前的測試項目(代碼、名稱)與開啟前 / 開啟後圖例。
  - **UAV 測試路徑**(兩台電視寬):同一條航線疊出兩趟軌跡與無人機目前位置;
    下方是目前這趟的任務進度(百分比)與目前階段。
  - **UAV 吞吐量開啟前後對比**:場域內只觀察這台 UAV。下行、上行吞吐量各一張折線圖
    (x 為路徑進度,兩趟對照),標題列顯示兩趟平均與變化量。
- 版面文字避開電視拼接縫。
- RIC 沒有測試在跑、且在 RIC 上一次執行結束**之後**才選這項,中牆才會切過來
  (否則停在 RIC 上一次的判決)。
- 版面規劃:`docs/外部文件/前端UI建議/2026-09-13_智慧網路實驗室_室外UAV情境中牆UI規劃.png`。

### 室內 AMR 情境 — `/indoor-scenario`
與室外 UAV 情境同一套畫面(**同樣尚未串接,前端靜態假資料**),載具換成室內 AMR,
在廠房走道沿同一條路徑跑兩趟(優化開啟前 / 開啟後)。差異:
- **左 · 即時環境影像**:4 路串流(2×2),名稱暫定為廠房攝影機 1~3、AMR 車載攝影機。
- **右 · 測試狀態總覽**:AMR 測試路徑 | 中間一欄上半**場域 UE 吞吐量**(10 台 UE 折線圖)、
  下半**行駛狀態**(行駛速度折線圖,電量以數值顯示)| 訊號狀態。
- 任務進度、訊號狀態折線圖與室外相同。
